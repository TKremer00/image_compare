...
pub(crate) struct FullHashPruner<'r, H: Hasher + Default>(&'r mut ImageReader<H>);

impl<'r, H: Hasher + Default> FullHashPruner<'r, H> {
    ...
    fn prune_internal<'a>(
        ...
        let hash = self.0.read_file(image.path)?;
...
impl<'r, H: Hasher + Default> IImagePruner for FullHashPruner<'r, H> {
    fn prune<'a>(
        mut self,
